...
!
  ...
  CONTAINS
    ...
    use prescribed_wv,       only: init_prescribed_wv_restart
    ...
    if ( .not. adiabatic .and. .not. ideal_phys )then
       ...
       call init_prescribed_wv_restart(File)
...
!
  ...
  subroutine write_restart_physics (File, cam_out, pbuf2d)
      ...
      use prescribed_wv,      only: write_prescribed_wv_restart
      use radiation,          only: radiation_do
      ...
      if ( .not. adiabatic .and. .not. ideal_phys )then
         ...
         call write_prescribed_wv_restart(File)
...
!#######################################################################
    ...
    subroutine read_restart_physics (File, cam_out,pbuf2d)
     ...
     use prescribed_wv,      only: read_prescribed_wv_restart
     !
     ! Arguments
     !
     type(file_desc_t), intent(inout) :: File
     ...
     if ( .not. adiabatic .and. .not. ideal_phys )then
        ...
        call read_prescribed_wv_restart(File)
