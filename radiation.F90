...
use prescribed_wv,   only: has_prescribed_wv
...
!-----------------------------------------------------------------------
    ...
    ! Get the prescribed wv index
    if (has_prescribed_wv) then
      i_Q        = pbuf_get_index('Q_in',errcode=err)
    end if
    ...
    ! Specific humidity needed for radiation
    call addfld ('SP_HUM','kg/kg',pver, 'I','Prescribed specific humidity for radiation',phys_decomp)
    !call addfld ('SP_HUM_SAV','kg/kg',pver, 'I','Origional specific humidity for radiation',phys_decomp)
...
!===============================================================================
  ...
  subroutine radiation_tend(state,ptend, pbuf, &
    ...
    !real(r8), pointer, dimension(:,:) :: sp_hum_sav ! specific humidity
...
!----------------------------------------------------------------------
    ...
    if (dosw .or. dolw) then
       ...
       if (has_prescribed_wv) then
         i_Q        = pbuf_get_index('Q_in')
         call pbuf_get_field(pbuf, i_Q, sp_hum)
         call outfld('SP_HUM',sp_hum,pcols,lchnk)
       else
         call rad_cnst_get_gas(0,'H2O', state, pbuf,  sp_hum)
       end if

       ! !for test, when do the real simulation, delete these codes
       ! call rad_cnst_get_gas(0,'H2O', state, pbuf,  sp_hum_sav)
       ! call outfld('SP_HUM_SAV',sp_hum_sav,pcols,lchnk)
